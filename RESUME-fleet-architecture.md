...
1. **Unlock the open emit on real axon** — wire ONE real open cross-host quirk (central persist-claim collector reading `config.users.users.<svc>.uid`, §8b Form C) with a SCOPED `pipe.collect`; measure affordable TODAY (laziness). First thing with value den lacks.
2. **Seam S2/S1** — `pipe.reads` cone-expander + unscoped-collectAll+config-dep lint + kill global flag.
...
## Commit status
- papers `main`: e3c spec committed (8a18a1f, 3202f0b, d570ef2). **Fleet architecture spec + this RESUME = UNCOMMITTED.** Per feedback: write specs to papers, don't auto-commit docs unless asked.
- No code changes anywhere (all design + /tmp probes).
