...
2. **Seam S1 ✅ DONE 2026-06-28 / S2 NEXT.** **S1 (kill the global flag)** — den commit `b3449c8b` on `feat/s1-per-sid-hostconfig` (off the harness's `fix/broadcast-home-pool-to-host`, NOT pushed; HOLD pending D5). `hasAnyConfigThunk`-gated `hostConfigs` → `hostConfigScopeIds` attrset + lazy `hostConfigFor`; `anyConfigDepThunk` gates only `bprimeEdges` (closed fleets zero-cost). den CI 1052→1054 zero-regression; synth + real-axon (axon-02/03) toplevel drvPath BYTE-IDENTICAL (validated github-pin vs `--override-input den path:<worktree>`). Plan+evidence: `plans/2026-06-28-fleet-seam-s1-per-sid-hostconfig.md`, `analysis/experiments/synthetic-fleet/s1-result.md`. **S2 ✅ DONE 2026-06-28 (full scope, user-chosen)** — den commit `487cc671` on `feat/s2-pipe-reads` (stacked on S1, held). `pipe.reads [paths]` cone-expander (`coneView` enforcement, NOT a runtime cut — laziness already scopes, base-dominated) + the unscoped-`collect`/`collectAll`+config-dep LINT (the keeper). den CI 1054→1057; lint PROVEN on the real `persist-claims` open emit; full declare→cone-restrict→byte-identical loop (axon-02/03 toplevel identical to pin). Plan+evidence: `plans/2026-06-28-fleet-seam-s2-pipe-reads-cone-lint.md`, `analysis/experiments/synthetic-fleet/s2-result.md`. **═══ The open-emit-affordability band (Plane-2a + S1 + S2) is COMPLETE.** Beyond it: the den-hoag inject-at-`instantiate` seam itself (§8a, where Plane-2a's saving is realized) + Plane-2b (deferred).
3. **Plane 2a class-share** — ✅ **PoC COMPLETE 2026-06-28 (first hard number).** `synth-measure/2a-share.sh`+test (commit `fa4d2864`, worktree), archetype-once + per-host axis-delta at units level, zero-exclusion parity gate, byte-identical. MEASURED on the 96-host agent class: **per-host eval-work marginal 21.72M → 8.76M copies (59.68% collapse), scale-invariant to the digit (class-size 6 & 46); 583M copies saved on a 46-host class, ~1.23B extrapolated at 96.** Residual = per-host module-merge spine (Gate-B bound). N=96 whole-fleet force OOMs (memory-bound finding). Plan+evidence: `plans/2026-06-28-plane-2a-class-share-poc.md`, `analysis/experiments/synthetic-fleet/2a-{baseline,result}.md`. STILL OPEN (deferred): extend from units to full toplevel cone; production wiring = the den-hoag inject-at-instantiate seam.
